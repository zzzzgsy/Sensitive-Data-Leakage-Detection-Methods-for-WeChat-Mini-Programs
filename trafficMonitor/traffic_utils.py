...
import requests
# 确保 NLTK 资源就绪（兼容 NLTK 3.8+）
# 放在报告/分析入口最前面（调用 nltk 之前）
import os, nltk

# 1) 显式加入用户目录（你下载到这）
USER_NLTK = os.path.join(os.path.expanduser("~"), "AppData", "Roaming", "nltk_data")
if USER_NLTK not in nltk.data.path:
    nltk.data.path.insert(0, USER_NLTK)

# 2)（可选）项目内置目录，离线更稳
LOCAL_NLTK = os.path.join(os.path.dirname(__file__), "nltk_data")
os.makedirs(LOCAL_NLTK, exist_ok=True)
if LOCAL_NLTK not in nltk.data.path:
    nltk.data.path.insert(0, LOCAL_NLTK)

# 3) 确保两个包都可用（兼容 NLTK 3.8+）
for pkg in ("punkt", "punkt_tab"):
    try:
        nltk.data.find(f"tokenizers/{pkg}")
    except LookupError:
        nltk.download(pkg, download_dir=LOCAL_NLTK)
...
def detect_privacy_test(response_content: dict):
    # 复用 detect_privacy_keys 获取所有字段
    # detect_privacy_keys 返回的是 [(key, value), ...] 的所有叶子节点
    all_leafs = analyze_obj(response_content)
    
    detected = []
    for key, value in all_leafs:
        # 对每个 key 进行 NLP/规则 判断
        p_key, p_category = judge_key(key)
        if p_category: # 如果识别出了隐私类别
            detected.append((key, p_category)) # 记录 (字段名, 类别)
...
privacy_set = {
    ...
    'doorcode': ['doorcode'],
    'userid': [ 'useruuid', 'useridx', 'userid', 'uid'],  # 这里为了把我们的case包含进来做了一些调整
    'loginid': ['loginid', 'loginuserid'],
    'speed': ['speed'],
    'habit': ['habit'],
    'tax': ['tax', 'shippingtax'],
    ...
    # ======== 以下为你新增的医疗类敏感词汇 ========
    'patient': ['patient', 'patientid', 'patientname', 'patientinfo'],
    'medical': ['medical', 'hospital', 'clinic', 'doctor', 'physician', 'healthcare'],
    'disease': ['disease', 'symptom', 'diagnosis', 'syndrome'],
    'treatment': ['treatment', 'prescription', 'medication', 'pharmacy'],
    ...
    # 'encrypteddata':['encrypteddata', 'encrypted_data'],
    # 'iv':['iv', 'initialization_vector'],
}
...
human_related_categories = [
    ...
    'wish', 'user',
    'patient', 'medical', 'disease', 'treatment', 'sessionkey', 'appsecret', 'accesstoken', 'encrypteddata', 'iv' # <--- 新增
]

# ======== 1. 新建系统级高危凭证专属规则字典 (快车道专用) ========
system_credential_set = {
...
def match_system_credential(key):
    """
    系统凭证专属匹配逻辑：
    直接转小写匹配，一旦发现命中规则库中的任意字符串，立刻返回其类别。
    """
    lower_key = key.lower()
    for cate, aliases in system_credential_set.items():
        for alias in aliases:
            # 去除下划线等符号进行纯字符比较，或者直接子串匹配
            if alias.replace('_', '') in lower_key.replace('_', ''):
                return cate
    return None
# 3. 本土化拼音映射字典
pinyin_map = {
...
def profile_obj_judge(key_list):
    ...
    # return 0, None
"""
def normalize_pinyin_to_english(field_name):
    # 将拼音强行替换为英文词根
    for pinyin, eng in pinyin_map.items():
        if pinyin in field_name:
            field_name = field_name.replace(pinyin, eng)
    return field_name
...
def judge_key(key):
    # ==========================================================
    # 赛道一（快车道）：系统级凭证匹配 (规则直达，不进 NLP)
    # ==========================================================
    sys_cred_category = match_system_credential(key)
    if sys_cred_category:
        # 一旦命中 session_key、iv 等，直接返回结果，短路拦截！
        return key, sys_cred_category
    # ==========================================================
    # 赛道二（慢车道）：应用层业务数据匹配 (拼音映射 + 字典 + NLP)
    # ==========================================================
...
# def judge_key(key):
#     # 第一步：先做拼音到英文的标准化映射！
#     normalized_key = normalize_pinyin_to_english(key)
    
#     # 后续完全按你原来的逻辑走，但此时 NLP 见到的已经是 patient_id 了
#     match_res = match_privacy_in_word_sequence(normalized_key)
#     dp_res = privacy_dependence_analyze(normalized_key, match_res)
#     return key, dp_res
...
def match_privacy_in_word_sequence(field):
    ...
    # print("match_privacy_in_word_sequence")
